...
shinyServer(function(input, output,session) {
  ...
  output$SalesPlot <- renderPlot({
    environment<-environment()
    sales = subset(sales,as.Date(as.character(Date.Created)) >= input$dateRange[1]&as.Date(as.character(Date.Created)) <= input$dateRange[2])
  ...
  output$SalesPlot_by_week <- renderPlot({
    environment<-environment()
    ...
    p <- ggplot(sales.week,aes(x=sales.week[,1], y= sales.week[,2])
                ,environment = environment)
    p <- p+ geom_line(size = 1.5)+geom_point(size=4, shape=21, fill="white")+
      ggtitle(paste("Sales Generated by",input$y.factor))+
      xlab("Week")+
